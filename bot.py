...
from config import (
    ...
    GOOGLE_B64_FILE, GOOGLE_CREDENTIALS_FILE,
...
def ensure_service_account_file():
    """Декодирует sa.b64 → service_account.json (если b64 есть)."""
    import base64
    b64_path = Path(GOOGLE_B64_FILE)
    out_path = Path(GOOGLE_CREDENTIALS_FILE)
    if b64_path.exists():
        raw = b64_path.read_text(encoding="utf-8").strip()
        data = base64.b64decode(raw)
        out_path.write_bytes(data)
        print(f"[DRIVE] Ключ декодирован из {GOOGLE_B64_FILE} → {GOOGLE_CREDENTIALS_FILE}")
    elif not out_path.exists():
        raise FileNotFoundError(
            f"Нет ни {GOOGLE_B64_FILE}, ни {GOOGLE_CREDENTIALS_FILE}"
        )
